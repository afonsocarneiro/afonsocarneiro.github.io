...
function update() {
	...
	const handleUpdate = event => {
		...
		axios
			.post('http://localhost:5173/users' + id, values)
			.then(res => {
				console.log(res);
				navigate('/');
			})
			...
			<div className='w-50 border bg-white  shadow px-5 pt-3 pb-5 rounded'>
				...
				<form onSubmit={handleUpdate}>
					...
					<div className='mb-2'>
						...
						<input type='text' name='email' className='form-control' placeholder='Enter Email' value={values.email} onChange={e => setValues({ ...values, email: e.target.value })} />
					</div>
					<div className='mb-2'>
